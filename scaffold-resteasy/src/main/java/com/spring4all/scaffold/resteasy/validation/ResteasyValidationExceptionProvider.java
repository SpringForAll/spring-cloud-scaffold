...
public class ResteasyValidationExceptionProvider implements ExceptionMapper<ValidationException> {
    ...
    public Response toResponse(ValidationException e) {
        ...
        if(e instanceof ParamValidationException) {
            logger.error("param validation exception caused, the exception message is {}", e.getMessage());
        ...
        if(e instanceof ResteasyViolationException) {
            ...
            logger.error("resteasy violation exception caused, the exception message is {}", e.getMessage());
            ...
            if (null != resteasyViolationException.getReturnValueViolations() && resteasyViolationException.getReturnValueViolations().size() == 0) {
                return buildResponse(Status.BAD_REQUEST, baseResult);
            } else {
                return buildResponse(Status.INTERNAL_SERVER_ERROR, baseResult);
            }
