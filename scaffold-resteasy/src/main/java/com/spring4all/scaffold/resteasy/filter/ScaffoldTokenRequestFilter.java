...
public class ScaffoldTokenRequestFilter implements ClientRequestFilter {
    ...
    public void filter(ClientRequestContext requestContext){
        ...
        if(null == tokenIgnore) {
            ...
            if(token == null) {
                LOGGER.error("Invoke restful without Token header. RequestURI: [{}]", ((PostMatchContainerRequestContext) requestContext).getUriInfo().getPath());
                throw new AuthorizationException(BaseErrorCode.FORBIDDEN.getCode(), "Token is null,please apply a token!");
            }
        }
    }
}
